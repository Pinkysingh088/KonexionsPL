...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_vendor'])) {
  ...
  $insertVendor = "INSERT INTO it_vendors (vertical_id, vendor_name) VALUES ($vertical_id, '$vendor_name')";
  if ($conn->query($insertVendor) === TRUE) {
      $vendor_id = $conn->insert_id;

      // Step 2: Insert expense
      $insertExpense = "INSERT INTO it_expenses (vendor_id, process_id, vertical_id, amount, start_date, end_date) 
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_vendor'])) {
  ...
  $processInfoQuery = "SELECT process_id, start_date FROM it_expenses WHERE vendor_id = $vendor_id LIMIT 1";
  ...
  if ($processInfoResult->num_rows > 0) {
    ...
    $conn->query("DELETE FROM it_expenses WHERE vendor_id = $vendor_id");

    // Step 3: Delete the vendor itself
    $sql = "DELETE FROM it_vendors WHERE vendor_id = $vendor_id AND vertical_id = $user_vertical_id";
  ...
  <form method="POST">
    <div class="form-group">
      ...
      <select name="vendor_id" required>
        ...
        <?php
        $vendors = $conn->query("SELECT vendor_id, vendor_name FROM it_vendors WHERE vertical_id = $user_vertical_id");
