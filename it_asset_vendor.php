...
// If vertical_id is not set, redirect to login
if (!$vertical_id) {
...
// Fetch vendors for the vertical
$vendors = [];
$query = "SELECT vendor_id, vendor_name FROM it_vendors WHERE vertical_id = $vertical_id";
$result = mysqli_query($conn, $query);
while ($row = mysqli_fetch_assoc($result)) {
    $vendors[] = $row;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Calculate total once by summing all entered amounts
    foreach ($vendor_amounts as $vendor_id => $amount) {
        if (is_numeric($amount) && $amount > 0) {
            $it_total += floatval($amount);
        }
    }

    // Loop through each vendor and its corresponding amount
    foreach ($vendor_amounts as $vendor_id => $amount) {
        if (is_numeric($amount) && $amount > 0) {
            // Check if vendor is already in the it_vendors table for this vertical
            $check_vendor_sql = "SELECT * FROM it_vendors WHERE vendor_id = ? AND vertical_id = ?";
            $stmt = $conn->prepare($check_vendor_sql);
            $stmt->bind_param("ii", $vendor_id, $vertical_id);
            $stmt->execute();
            $result = $stmt->get_result();  

            // If vendor doesn't exist, insert into it_vendors
            if ($result->num_rows == 0) {
                $insert_vendor_sql = "INSERT INTO it_vendors (vertical_id, vendor_name) 
            ...
            // Insert or update entry in it_expenses table for this vendor
            $check_expense_sql = "SELECT * FROM it_expenses WHERE vendor_id = ? AND process_id = ? AND start_date = ? AND end_date = ?";
            ...
            if ($result->num_rows > 0) {
                ...
                $update_sql = "UPDATE it_expenses 
                               SET amount = ?, it_total = ? 
                               WHERE vendor_id = ? AND process_id = ? AND start_date = ? AND end_date = ?";
                $stmt = $conn->prepare($update_sql);
                $stmt->bind_param("diiiss", $new_amount, $new_total, $vendor_id, $process_id, $start_date, $end_date);
                $stmt->execute();
            
            

            } else {
                // Insert new record for this vendor and process
                $insert_sql = "INSERT INTO it_expenses (vendor_id, vertical_id, process_id, amount, start_date, end_date, it_total) 
                               VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($insert_sql);
                $stmt->bind_param("iiidsss", $vendor_id, $vertical_id, $process_id, $amount, $start_date, $end_date, $it_total);
                $stmt->execute();
            }
        }
    }
    ...
    header("Location: it_asset_vendor.php?msg=IT expenses saved successfully");
    exit;
}
...
<head>
    ...
    <title>IT Asset Expense Entry</title>
    <style>
                body {
    ...
    <script>
        function calculateTotal() {
            ...
            document.getElementById('it_total').value = total.toFixed(2);
        }
    </script>
</head>
<body>
...
<div class="form-container">
    <h2>IT Asset Expense Entry</h2>
    <form method="post" action="it_asset_vendor.php">
        <div class="grid">
            <div>
        ...
        <div class="grid" style="margin-top: 20px;">
            <div>
                <label>Total</label>
                <input type="number" id="it_total" name="it_total" readonly>
            </div>
        </div>
...
<!---pop up message for successfully---->
<?php if (isset($_GET['msg'])): ?>
<script>
    alert("<?= htmlspecialchars($_GET['msg']) ?>");
    // Remove the query string so the alert doesn't show again on reload
